...
export default function PingModal({ owner, onClose }) {
  ...
  async function recordPing(method) {
    // Save ping to Supabase via API
  ...
  async function handleWhatsApp() {
    setSending(true)
    try {
      await recordPing('whatsapp')
      const num  = (owner.whatsapp || owner.phone || '').replace(/\D/g, '')
  ...
  async function handleSMS() {
    ...
    try {
      ...
      await recordPing('sms')
      if (!res.ok) {
        // Fallback: native SMS app
        const body = encodeURIComponent(`[QrCar] ${selectedReason?.label || ''} — ${message}`)
        window.open(`sms:${owner.phone}?body=${body}`, '_blank')
      }
      setStep('sent')
    } catch {
        ...
        <motion.div className="relative w-full max-w-md rounded-2xl p-6 z-10"
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h3 className="font-display font-700 text-xl" style={{ color: 'var(--text)' }}>{owner.name}</h3>
            </div>
            <button onClick={onClose}
              className="w-8 h-8 rounded-full flex items-center justify-center text-lg"
              style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--sub)', border: 'none', cursor: 'pointer' }}>
              ×
            </button>
          </div>
                  ...
                  <button key={r.id}
                    ...
                    {r.urgent && (
                      <span className="badge text-[10px] px-2 py-0.5 rounded-full"
                        style={{ background: 'rgba(255,92,0,0.15)', color: 'var(--orange)' }}>URGENT</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          {step === 'message' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              ...
              <div>
                ...
                <input className="input-field" placeholder="e.g. Karim"
                  value={senderName} onChange={e => setSender(e.target.value)} />
              </div>

              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--sub)' }}>Add a note (optional)</label>
                <textarea className="input-field resize-none" rows={3}
                  placeholder="e.g. I'm parked right behind you in the blue Honda..."
              ...
              <div className="grid grid-cols-3 gap-2">
                {/* In-App */}
                <button onClick={handleInApp} disabled={sending}
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-xs font-medium"
                  style={{ background:'rgba(0,212,255,0.08)', border:'1px solid rgba(0,212,255,0.25)', color:'var(--cyan)', cursor:'pointer' }}>
                  {sending
                ...
                <button onClick={handleWhatsApp} disabled={sending}
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-xs font-medium"
                  style={{ background:'rgba(37,211,102,0.08)', border:'1px solid rgba(37,211,102,0.25)', color:'#25D366', cursor:'pointer' }}>
                ...
                <button onClick={handleSMS} disabled={sending}
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-xs font-medium"
                  style={{ background:'rgba(255,92,0,0.08)', border:'1px solid rgba(255,92,0,0.25)', color:'var(--orange)', cursor:'pointer' }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--sub)' }}>
                  {owner.name} has been notified and will respond shortly.
                </p>
              </div>
