...
export default function CarProfilePage() {
  ...
  if (notFound) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-6 text-center mesh-bg"
      ...
      <h1 style={{ fontFamily:'Barlow Condensed', fontSize:'32px', fontWeight:800 }}>Profile Not Found</h1>
      <p style={{ color:'var(--sub)' }}>This QR code may be invalid or deactivated.</p>
      <a href="/" className="btn-primary mt-2">← Go to QrCar</a>
    </div>
  )

  const waNumber = (owner.whatsapp || owner.phone || '').replace(/\D/g,'')
    ...
    <>
      ...
      <div className="min-h-screen mesh-bg" style={{ background:'var(--bg)', fontFamily:'DM Sans, sans-serif', color:'var(--text)' }}>
        ...
        <motion.div variants={container} initial="hidden" animate="show"
          className="max-w-sm mx-auto px-4 pt-4 pb-12">

          {/* Profile card */}
          <motion.div variants={item} className="rounded-2xl overflow-hidden mb-4"
            style={{ background:'var(--card)', border:'1px solid var(--border)' }}>
            <div className="h-20 relative" style={{ background:'linear-gradient(135deg,rgba(0,212,255,0.12),rgba(255,92,0,0.08))' }}>
              <div className="absolute -bottom-8 left-5">
                <img src={owner.avatar} alt={owner.name} className="w-16 h-16 rounded-full"
                  style={{ border:'3px solid var(--card)', background:'var(--surface)' }} />
              </div>
            </div>
            <div className="pt-10 pb-5 px-5">
              <h1 style={{ fontFamily:'Barlow Condensed', fontSize:'28px', fontWeight:800, lineHeight:1.1, marginBottom:'4px' }}>{owner.name}</h1>
          ...
          <motion.div variants={item} className="flex justify-center mb-4">
            <div className="px-6 py-3 rounded-xl" style={{ background:'var(--card)', border:'2px solid var(--border)' }}>
              <p style={{ fontFamily:'Barlow Condensed', fontSize:'30px', fontWeight:800, letterSpacing:'0.15em', textAlign:'center' }}>
                {owner.plate}
              </p>
            </div>
          </motion.div>
          ...
          <motion.div variants={item} className="mb-4">
            <button onClick={() => setShowPing(true)}
              className="w-full py-4 rounded-2xl text-base font-semibold flex items-center justify-center gap-3 glow-orange"
              style={{ background:'var(--orange)', color:'#fff', border:'none', cursor:'pointer' }}>
          ...
          <motion.div variants={item} className="grid grid-cols-3 gap-2 mb-4">
            <a href={`https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi ${owner.name}! I found your QrCar profile.`)}`}
              target="_blank" rel="noreferrer"
              className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-xs font-medium no-underline"
          ...
          {/* Vehicle info */}
          <motion.div variants={item} className="rounded-2xl p-4 space-y-1"
            style={{ background:'var(--card)', border:'1px solid var(--border)' }}>
            <p className="text-xs font-medium tracking-widest uppercase mb-3" style={{ color:'var(--sub)' }}>Vehicle Info</p>
            {[['Owner', owner.name],['Vehicle', owner.car],['Plate', owner.plate]].map(([l,v]) => (
              <div key={l} className="flex items-center justify-between py-2" style={{ borderBottom:'1px solid var(--border)' }}>
                <span className="text-xs" style={{ color:'var(--sub)' }}>{l}</span>
      ...
      {showPing && <PingModal owner={owner} onClose={() => setShowPing(false)} />}
    </>
  )
}
